...
class MyDataset(Dataset):
    def __init__(self, norm, inputs_bool, device, which, noise=0., samples=4096, data_path="data/Poisson_n70_L20_w2pi_FullDtN.h5"):
        ...
        self.L = self.reader[self.which]['sample_0']["input_dbc"][:].shape[1] # Number of DBC, NBC pairs per conductivity image

        self.inp_dbc_dim_branch = 276
        self.inp_nbc_dim_branch = 272
        self.n_fun_samples = self.L
    ...
    def __getitem__(self, index):
        ...
        inputs_dbc = inputs_dbc.view(1, 4, 69, self.L).permute(3, 0, 1, 2)
        inputs_dbc_trim = inputs_dbc[:,:,:,:68]
        inputs_nbc = inputs_nbc.view(1, 4, 68, self.L).permute(3, 0, 1, 2)

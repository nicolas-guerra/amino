...
folder = sys.argv[1]
data_path = sys.argv[5] # Training and Testing data path
test_noise = 0

p = 1 # L1 or L2 error
error_type = "relative" # can be "relative" or "absolute"

freq_print = 1

if len(sys.argv) == 6: # changed from 5 to 6 since now I input data_path
...
train_dataset = MyDataset(norm=norm, inputs_bool=inputs_bool, device=device, which="training", mod=mod,data_path=data_path)
# Note that noise was originally set to 0.1
test_dataset = MyDataset(norm=norm, inputs_bool=inputs_bool, device=device, which="validation", mod=mod, noise=test_noise, data_path=data_path)
inp_dim_branch = train_dataset.inp_nbc_dim_branch
...
for epoch in range(start_epoch, epochs + start_epoch):
    ...
    with bar as tepoch:
        ...
        for step, (input_concat_batch, output_batch) in enumerate(training_set):
            ...
            tepoch.update(1)
            input_concat_batch = input_concat_batch.to(device, non_blocking=True)
            output_batch = output_batch.to(device, non_blocking=True)
            cuda_debugger.print("Loading")

            pred_train = model(input_concat_batch, grid)

            # Denormalize to compute training error
            # pred_train = train_dataset.denormalize(pred_train)
            # output_batch = train_dataset.denormalize(output_batch)

            cuda_debugger.print("Forward")

            if error_type == "relative":
                loss_batch = train_loss(pred_train, output_batch) / torch.mean(abs(output_batch) ** p) ** (1 / p)
            elif error_type == "absolute":
                loss_batch = train_loss(pred_train, output_batch)
            else:
                raise ValueError("Choose error_type = 'relative' or error_type = 'absolute'.")
            if reg_param != 0:
                loss_batch += reg_param * model.regularization(reg_exponent)
            ...
            # Evaluation
            ########################################################################################
            # Average across loss across batches as we go
            running_train_error = running_train_error * step / (step + 1) + loss_batch.item() / (step + 1)
            tepoch.set_postfix({'Batch': step + 1,
                                'Train loss (in progress)': running_train_error,
            ...
            # Check if training tolerance is reached
            # UNCOMMENT BELOW TO STOP WHEN CERTAIN TOLERANCE REACHED
            # CURRENTLY STOPS AFTER CERTAIN NUMBER OF EPOCHS
            # if running_relative_train_mse < .01:
            #     train_tol_reached = True
            #     break

        if p == 1 and error_type == 'relative':
            writer.add_scalar("train_loss/Relative L1 Error", running_train_error, epoch)
        if p == 1 and error_type == 'absolute':
            writer.add_scalar("train_loss/Absolute L1 Error", running_train_error, epoch)
        if p == 2 and error_type == 'relative':
            writer.add_scalar("train_loss/Relative L2 Error", running_train_error, epoch)
        if p == 2 and error_type == 'absolute':
            writer.add_scalar("train_loss/Absolute L2 Error", running_train_error, epoch)
        end = timer()
        elapsed = end - start
        ########################################################################################
        # Evaluation
        ...
        if epoch % freq_print == 0:
            if not full_training:
                running_test_error = 0.0

                model.eval()
                with torch.no_grad():
                    for step, (input_concat_batch, output_batch) in enumerate(testing_set):
                        input_concat_batch = input_concat_batch.to(device, non_blocking=True)
                        output_batch = output_batch.to(device, non_blocking=True)
                        pred_test = model(input_concat_batch, grid)
                        ...
                        loss_batch = test_loss(pred_test, output_batch) / torch.mean(abs(output_batch) ** p) ** (1 / p)
                        # running_relative_test_mse = running_relative_test_mse * step / (step + 1) + loss_test.item() ** (1 / p) * 100 / (step + 1)
                        running_test_error = running_test_error * step / (step + 1) + loss_batch.item() / (step + 1)

                    if p == 1 and error_type == 'relative':
                        writer.add_scalar("val_loss/Relative L1 Error", running_test_error, epoch)
                    if p == 1 and error_type == 'absolute':
                        writer.add_scalar("val_loss/Absolute L1 Error", running_test_error, epoch)
                    if p == 2 and error_type == 'relative':
                        writer.add_scalar("val_loss/Relative L2 Error", running_test_error, epoch)
                    if p == 2 and error_type == 'absolute':
                        writer.add_scalar("val_loss/Absolute L2 Error", running_test_error, epoch)

            else:
                running_test_error = running_train_error

            if running_test_error < best_model_testing_error:
                best_model_testing_error = running_test_error
                torch.save(model, folder + "/model.pkl")
                writer.add_scalar("val_loss/Best Testing Error", best_model_testing_error, epoch)
        ...
        with open(folder + '/errors.txt', 'w') as file:
            file.write("Error Type: " + error_type + "L" + str(p) + "\n")
            file.write("Training Error: " + str(running_train_error) + "\n")
            file.write("Testing Error: " + str(running_test_error) + "\n")
            file.write("Best Testing Error: " + str(best_model_testing_error) + "\n")
            file.write("Current Epoch: " + str(epoch) + "\n")
            file.write("Time per Epoch: " + str(time_per_epoch) + "\n")
            file.write("Workers: " + str(max_workers) + "\n")

        # Storing the epoch number, training error, and testing error
        # Check if error_table.csv file already exists
        if os.path.isfile(folder + "/error_table.csv"):
            with open(folder+'/error_table.csv', 'a') as f:
                # using csv.writer method from CSV package
                write = csv.writer(f)
                write.writerow([epoch, running_train_error, running_test_error])
        else:
            # Export error_table to csv
            fields = ['Epoch', 'Training Error', 'Testing Error'] # field names 
            with open(folder+'/error_table.csv', 'w') as f:
                # using csv.writer method from CSV package
                write = csv.writer(f)
        ...
        tepoch.set_postfix({"Val loss": running_test_error})
        tepoch.close()

    # if counter > patience:
    #     print("Early stopping:", epoch, counter)
    #     break
    # Instead of using patience, I will stop once the "running relative training mse" is less than .01 (1 percent)
    if train_tol_reached == True:
        print("Stopping: 1% Tolerance Reached")
        break

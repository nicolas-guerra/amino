# AMINO is a variant of NIO credited to Molinaro et al. https://arxiv.org/pdf/2301.11167
...
data_path = sys.argv[3] # Training and Testing data path
test_noise = 0
...
# Note that branch architecture is not used in NIOHelmPermInv
...
# Note that denseblock architecture is not used in NIOHelmPermInv
...
train_dataset = MyDataset(norm=norm, inputs_bool=inputs_bool, device=device, which="training", data_path=data_path)
# Note that noise was originally set to 0.1
test_dataset = MyDataset(norm=norm, inputs_bool=inputs_bool, device=device, which="validation", noise=test_noise, data_path=data_path)
